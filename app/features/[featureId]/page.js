            ...
            <section className="page-card">
                <div className="card-heading">
                    <div>
                        <p className="small-heading">
                            <Link href={`/projects/${feature.projectId}`}>{feature.project.name}</Link> · Feature
                        </p>
                        <h1>{feature.title}</h1>
                    </div>
                    ...
                    {edit === "1" ? (
                        ...
                        <Link href={`/features/${feature.id}?edit=1`} className="main-button">
                            Edit
                        </Link>
                    )}
                </div>
                <div className="details-layout">
                    <section className="details-box">
                        <h2>Details</h2>
                        {edit === "1" ? <FeatureForm feature={feature} /> : <FeatureDetails feature={feature} />}
                        <h3>Linked issues</h3>
                        <ul className="item-list">
                            {feature.issues.map((issue) => (
                                <li className="item-box" key={issue.id}>
                                    <Link href={`/issues/${issue.id}`}>{issue.title}</Link> <span className="tag">{issue.status.replace("_", " ")}</span>
                                </li>
                            ))}
    ...
    return (
        <>
            <dl className="read-only-fields">
                <div>
                    <dt>Status</dt>
                    <dd>{feature.status.replace("_", " ")}</dd>
                </div>
                <div>
                    <dt>Priority</dt>
                ...
                <div>
                    <dt>Due date</dt>
                    <dd>{feature.dueDate?.toLocaleDateString() ?? "None"}</dd>
                </div>
                <div>
                    <dt>Labels</dt>
                    <dd className="tag-row">
                        ...
                        {!feature.featureLabels.length ? "None" : null}
                    </dd>
                </div>
            </dl>
    ...
    return (
        <form action={updateFeatureAction}>
            ...
            <div className="form-fields">
                ...
                <label className="form-field">
                    <span>Priority</span>
                    <select name="priority" defaultValue={feature.priority}>
                ...
                <label className="form-field">
                    <span>Assignee</span>
                    <select name="assignedTo" defaultValue={feature.assignedTo ?? ""}>
                        <option value="">Unassigned</option>
                        {feature.project.members.map(({ user }) => (
            ...
            <fieldset className="label-options">
                ...
                {feature.project.labels.map((label) => (
                    <label key={label.id}>
...
function Discussion({ comments, featureId }) {
    ...
    return (
        <section className="details-box">
            ...
            <form action={addFeatureCommentAction}>
